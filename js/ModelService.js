...
model.factory('modelService', function () {
    ...
    var guestList = {
        list: [],
        add: function (name, phone, gender) {//添加方法
            // 判断是否可以添加
            var isok = true;
            //  1. 判断用户名或者手机号是否为空
            ...
            // 2.判断用户的手机是否合法，姓名是否合规[2-10]字符
            if (name.length<2||name.length>10||!(/^1[3456789]\d{9}$/.test(phone))) {
                return {
                    code: 2, // code值为1表示手机号或者用户名是空的
                    guest: null
                }
            }
            // 3. 判断用户的手机是否重复
        ...
        // 获取和状态码相同的所有用户的信息
        getList: function (state_gender) {
            if (state_gender == Guest.ALL) {
                return this.list.filter(function () {
                    return true;
                });
            }
            return this.list.filter(function (guest) {
                return guest.state == state_gender||guest.gender==state_gender;
            });
        }
    };
    return guestList;
